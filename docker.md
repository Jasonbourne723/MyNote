...
镜像是一种轻量级、可执行的独立软件包，用来打包软件运行环境和基于运行环境开发的软件。基于联合文件系统（UnionFS）技术，Docker 中的每一个镜像都是由一系列只读的层组成，Dockerfile 中的每一个命令都会在已有的只读层上创建一个新的层。当镜像被 `docker run `命令创建时就会在镜像的最上层添加一个可写的层，也就是容器层，所有对于运行时容器的修改其实都是对这个容器读写层的修改(写时拷贝)。
- 分层最大的一个优点是共享资源；
